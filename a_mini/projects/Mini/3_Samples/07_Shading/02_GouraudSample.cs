...
namespace PixelFarm.Agg.Sample_Gouraud
{
    ...
    public class GouraudApplication : DemoBase
    {
        ...
        public void RenderGourand(Graphics2D gx)
        {
            ...
            SpanGenGouraudRGBA gouraundSpanGen = new SpanGenGouraudRGBA();
            ...
            gouraundSpanGen.SetColor(ColorRGBA.Make(1, 0, 0, alpha),
                              ColorRGBA.Make(0, 1, 0, alpha),
                              ColorRGBA.Make(brc, brc, brc, alpha));
            gouraundSpanGen.SetTriangle(m_x[0], m_y[0], m_x[1], m_y[1], xc, yc, d);

            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen); 



            gouraundSpanGen.SetColor(ColorRGBA.Make(0, 1, 0, alpha),
                              ColorRGBA.Make(0, 0, 1, alpha),
                              ColorRGBA.Make(brc, brc, brc, alpha));

            gouraundSpanGen.SetTriangle(m_x[1], m_y[1], m_x[2], m_y[2], xc, yc, d);
            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen); 

            gouraundSpanGen.SetColor(ColorRGBA.Make(0, 0, 1, alpha),
                            ColorRGBA.Make(1, 0, 0, alpha),
                            ColorRGBA.Make(brc, brc, brc, alpha));
            gouraundSpanGen.SetTriangle(m_x[2], m_y[2], m_x[0], m_y[0], xc, yc, d);
            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen); 
            ...
            gouraundSpanGen.SetColor(ColorRGBA.Make(1, 0, 0, alpha),
                              ColorRGBA.Make(0, 1, 0, alpha),
                              ColorRGBA.Make(brc, brc, brc, alpha));
            gouraundSpanGen.SetTriangle(m_x[0], m_y[0], m_x[1], m_y[1], x1, y1, d); 
            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen);

            gouraundSpanGen.SetColor(ColorRGBA.Make(0, 1, 0, alpha),
                           ColorRGBA.Make(0, 0, 1, alpha),
                           ColorRGBA.Make(brc, brc, brc, alpha));
            gouraundSpanGen.SetTriangle(m_x[1], m_y[1], m_x[2], m_y[2], x2, y2, d); 
            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen);

            gouraundSpanGen.SetColor(ColorRGBA.Make(0, 0, 1, alpha),
                            ColorRGBA.Make(1, 0, 0, alpha),
                            ColorRGBA.Make(brc, brc, brc, alpha));
            gouraundSpanGen.SetTriangle(m_x[2], m_y[2], m_x[0], m_y[0], x3, y3, d); 
            painter.Fill(gouraundSpanGen.MakeVxs(), gouraundSpanGen);
        }
