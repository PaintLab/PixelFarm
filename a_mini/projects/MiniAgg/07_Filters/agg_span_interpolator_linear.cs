...
namespace MatterHackers.Agg.Lines
{
    ...
    public sealed class span_interpolator_linear : ISpanInterpolator
    {
        ...
        public void begin(double x, double y, int len)
        {
            ...
            int x1 = AggBasics.iround(tx * (double)subpixel_scale_e.subpixel_scale);
            int y1 = AggBasics.iround(ty * (double)subpixel_scale_e.subpixel_scale);

            tx = x + len;
            ty = y;
            m_trans.transform(ref tx, ref ty);
            int x2 = AggBasics.iround(tx * (double)subpixel_scale_e.subpixel_scale);
            int y2 = AggBasics.iround(ty * (double)subpixel_scale_e.subpixel_scale);
        ...
        public void resynchronize(double xe, double ye, int len)
        {
            ...
            m_li_x = new dda2_line_interpolator(m_li_x.y(), AggBasics.iround(xe * (double)subpixel_scale_e.subpixel_scale), (int)len);
            m_li_y = new dda2_line_interpolator(m_li_y.y(), AggBasics.iround(ye * (double)subpixel_scale_e.subpixel_scale), (int)len);
        }
