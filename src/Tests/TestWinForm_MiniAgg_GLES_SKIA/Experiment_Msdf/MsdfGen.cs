...
namespace ExtMsdfgen
{
    ...
    public static class MsdfGenerator
    {
        ...
        //public static void generateMSDF3(FloatRGBBmp output, Shape shape, double range, Vector2 scale, Vector2 translate, double edgeThreshold, BmpEdgeLut lut)
        //{
        //    List<Contour> contours = shape.contours;
        //    int contourCount = contours.Count;
        //    int w = output.Width;
                ...
                for (int x = 0; x < w; ++x)
                {
                    ...
                    if (useFake)
                    {
                        ...
                        if (b.minDistance < sb.minDistance)
                            sb = b;
                        //----------------
                        if (edgeStructure.AreaKind == AreaKind.Outside)
                        {
                            winding = -1;
                        }
                        else
                    ...
                    {
                        EdgeSegment ss = selectedSegment;
