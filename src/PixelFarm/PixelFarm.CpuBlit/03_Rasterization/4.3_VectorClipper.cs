...
namespace PixelFarm.CpuBlit.Rasterization
{
    ...
    partial class ScanlineRasterizer
    {
        class VectorClipper
        {
            ...
            {
                _ras = ras;
                _clipBox = new CartesRect(0, 0, 0, 0);
                _x1 = _y1 = _f1 = 0;
                _clipping = false;
            }
            public CartesRect GetVectorClipBox() => _clipBox;

            public void SetClipBox(int x1, int y1, int x2, int y2)
            {
                _clipBox = new CartesRect(x1, y1, x2, y2);
                _clipBox.Normalize();
                _clipping = true;
            }
            ...
            public void SetClipBoxWidthX3ForSubPixelLcdEffect(bool value)
            {
                ...
                if (value != _clipBoxWidthX3ForSubPixelLcdEffect)
                {
                    ...
                    if (value)
                    {
                        _clipBox = new CartesRect(_clipBox.Left, _clipBox.Bottom, _clipBox.Left + (_clipBox.Width * 3), _clipBox.Height);
                    }
                    else
                    {
                        //set back
