...
static int mswin_create_window2_psext(IVideo *self,HWND importHwnd)
{
	...
	//mswin_hwnd = self->hWnd = importHwnd;
	return 0;
}
...
static int mswin_init(IVideo *self, const char *windowid,HWND importHwnd)
{
	...
	if(mswin_create_window2_psext(self,importHwnd)!=0){
		return -1;
	}
	//-----------------------------------------------------
	...
	//mswin_handle_message = mswin_message_handle; //uncomment this if want native win handler
	//WIN_WinPaint = mswin_WinPaint;//uncomment this if use bitblt 
	return 0;
}
...
int ikitwin_init(HWND importHwnd, IVideo* winVideo)
{
	...
	#ifdef __unix
	retval = x11_init(winVideo, windowid);
	#else
	retval = mswin_init(winVideo, windowid,importHwnd);
	#endif
	...
	int w, int h, int bpp, int flag, IVideo* winVideo)
{
	...
	#ifdef __unix
	retval = x11_set_mode(winVideo, w, h, bpp, flag);
	#else
	retval = mswin_set_mode(winVideo, w, h, bpp, flag);
	#endif
	if (retval != 0) 
		return retval;	 
	winVideo->modeset =1;
...
void ikitwin_release_mode(IVideo* winVideo)
{
	...
	#ifdef __unix
	x11_release_mode(winVideo);
	#else
	mswin_release_mode(winVideo);
	#endif
...
int ikitwin_update(const int *rect, int n, IVideo* winVideo)
{
	...
	fullwindow[0] = 0;
	fullwindow[1] = 0;
	fullwindow[2] = winVideo->w;
	fullwindow[3] = winVideo->h;
	if (rect == NULL) {
		rect = fullwindow;
		if (n > 1) n = 1;
	}
	#ifdef __unix
	x11_update_image(winVideo, rect, n);
	#else
	mswin_update_image(winVideo, rect, n);
	#endif
	return 0;
}
...
int ikitwin_lock(void **pixel, long *pitch, IVideo* winVideo)
{
	int retval;
	#ifdef __unix
	retval = x11_lock_image(winVideo);
	#else
	retval = mswin_lock_image(winVideo);
	#endif
	if (retval == 0) {
		if (pixel) *pixel = winVideo->pixel;
		if (pitch) *pitch = winVideo->pitch;
	}
	return retval;
}
...
{ 
	...
	if (bmask) *bmask = winVideo->bmask;
	return winVideo->depth;
}
