...
int iscreen_init_main(int w, int h, int bpp,HWND importHwnd, IVideo* winVideo)
{
	...
	int fmt = 0;
	total01++;
	winVideo->iscreen_dx = (bpp < 0)? 0 : 1;

	bpp = (bpp < 0)? (-bpp) : bpp;
	if (winVideo->iscreen_dx) {
		if (DDrawInit() != 0) winVideo->iscreen_dx = 0;
	}
	//-------------------------------------------- 
	if (ikitwin_init(importHwnd,winVideo) != 0) return -1;
	if (bpp < 0 && winVideo->iscreen_dx) {
		winVideo->iscreen_dx = 0;
		bpp = -bpp;
		DDrawDestroy();
	}

	//set detail for import hwnd 
	if (ikitwin_set_mode(w, h, bpp, 0,winVideo) != 0) return -2;

	if (winVideo->iscreen_dx) {
		winVideo->dxdesktop = DDrawCreateScreen(importHwnd, 0, 0, 0, 0); 
		if (winVideo->dxdesktop  != NULL) {
			  
			winVideo->dxscreen = DDrawSurfaceCreate(w, h, PIXFMT_DEFAULT,
				DDSM_SYSTEMMEM, NULL);
			if (winVideo->dxscreen  != NULL) {
				void *bits;
				long pitch;
				DDrawSurfaceInfo(winVideo->dxscreen, 0, 0, &iscreen_bpp, &iscreen_fmt);
				if (DDrawSurfaceLock(winVideo->dxscreen, &bits, &pitch, 0) == 0) {
					int fmt;
					winVideo->iscreen = ibitmap_ref(bits, pitch, w, h, iscreen_bpp);
					fmt = ibitmap_pixfmt_guess(winVideo->iscreen);
					if (fmt == IPIX_FMT_A8R8G8B8) fmt = IPIX_FMT_X8R8G8B8;
					ibitmap_pixfmt_set(winVideo->iscreen, fmt);
					DDrawSurfaceUnlock(winVideo->dxscreen);
					iscreen_locked = 0;
				}	else {
					DDrawSurfaceRelease(winVideo->dxscreen);
					DDrawSurfaceRelease(winVideo->dxdesktop);
					winVideo->dxscreen = NULL;
					winVideo->dxdesktop = NULL;
					DDrawDestroy();
					winVideo->iscreen_dx = 0;
				}
			}	else {
	...
	if (winVideo->iscreen_dx == 0) {
		void *bits;
		long pitch;
		int fmt;
		ikitwin_lock(&bits, &pitch,winVideo);
		winVideo->iscreen = ibitmap_ref(bits, pitch, w, h, bpp);
		ikitwin_unlock(winVideo);
		fmt = ibitmap_pixfmt_guess(winVideo->iscreen);
		if (fmt == IPIX_FMT_A8R8G8B8) fmt = IPIX_FMT_X8R8G8B8;
		ibitmap_pixfmt_set(winVideo->iscreen, fmt);
		iscreen_bpp = bpp;
	...
	#endif
	winVideo->cscreen = ibitmap_create(w, h, bpp);
	fmt = ibitmap_pixfmt_guess(winVideo->cscreen);
	if (fmt == IPIX_FMT_A8R8G8B8) fmt = IPIX_FMT_X8R8G8B8;
	ibitmap_pixfmt_set(winVideo->cscreen, fmt);

	if (winVideo->iscreen->bpp == 8) {
		iscreen_index = (iColorIndex*)malloc(sizeof(iColorIndex));
		winVideo->iscreen->extra = iscreen_index;
		assert(iscreen_index);
		iscreen_init_index(iscreen_index);
	}
	if (winVideo->cscreen->bpp == 8) {
		cscreen_index = (iColorIndex*)malloc(sizeof(iColorIndex));
		winVideo->cscreen->extra = cscreen_index;
...
void iscreen_quit(IVideo* winVideo)
{
	...
	if (winVideo->iscreen) ibitmap_unref(winVideo->iscreen);
	if (winVideo->cscreen) ibitmap_release(winVideo->cscreen);
	total01--;
	...
	winVideo->iscreen = NULL;
	winVideo->cscreen = NULL;
	
	#ifndef __unix
	if (winVideo->iscreen_dx) {
		if (winVideo->dxscreen) DDrawSurfaceRelease(winVideo->dxscreen);
		if (winVideo->dxdesktop) DDrawSurfaceRelease(winVideo->dxdesktop);
		winVideo->dxscreen = NULL;
		winVideo->dxdesktop = NULL;
		DDrawDestroy();
	}
	#endif
	ikitwin_release_mode(winVideo);
	ikitwin_quit();
...
int iscreen_lock( IVideo* winVideo)
{
	...
	if (winVideo->iscreen_dx) {
		...
		if (iscreen_locked == 0) {
			if (DDrawSurfaceLock(winVideo->dxscreen, &bits, &pitch, 0) == 0) {
				ibitmap_adjust(winVideo->iscreen, bits, pitch);
			}	else {
				return -1;
			}
		}
...
int iscreen_update(int *rect, int n,IVideo* winVideo )
{
	...
	#endif
	//----------------------------------------
	ikitwin_update(rect, 1,winVideo);
	return 0;
}
...
int iscreen_convert(int *rect, int n, IVideo* winVideo)
{
	...
	for (i = 0; i < n; i++, rect += 4) {
		...
		int w = rect[2];
		int h = rect[3];
		ibitmap_convert(winVideo->iscreen, x, y, winVideo->cscreen, x, y, w, h, NULL, 0);
	}
	iscreen_unlock(winVideo);
	return 0;
}
